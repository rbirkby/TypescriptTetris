...
type Color = 'cyan' | 'blue' | 'orange' | 'yellow' | 'green' | 'purple' | 'red';
...
function html(id: string, html: string) {
...
const KEY = { ESC: 27, SPACE: 32, LEFT: 37, UP: 38, RIGHT: 39, DOWN: 40 },
  ...
  ucanvas = get('upcoming') as HTMLCanvasElement,
...
let dx: number,
  ...
  blocks: (Type | undefined)[][], // 2 dimensional array (nx*ny) representing tetris court - either empty block or occupied by a 'piece'
  ...
  current: Piece, // the current piece
  next: Piece, // the next piece
  score: number, // the current score
  vscore: number, // the currently displayed score (it catches up to score in small chunks - like a spinning slot machine)
  rows: number, // number of completed rows in the current game
  step: number; // how long before current piece drops by 1 row
...
const i: Type = { size: 4, blocks: [0x0f00, 0x2222, 0x00f0, 0x4444], color: 'cyan' };
const j: Type = { size: 3, blocks: [0x44c0, 0x8e00, 0x6440, 0x0e20], color: 'blue' };
const l: Type = { size: 3, blocks: [0x4460, 0x0e80, 0xc440, 0x2e00], color: 'orange' };
const o: Type = { size: 2, blocks: [0xcc00, 0xcc00, 0xcc00, 0xcc00], color: 'yellow' };
const s: Type = { size: 3, blocks: [0x06c0, 0x8c40, 0x6c00, 0x4620], color: 'green' };
const t: Type = { size: 3, blocks: [0x0e40, 0x4c40, 0x4e00, 0x4640], color: 'purple' };
const z: Type = { size: 3, blocks: [0x0c60, 0x4c80, 0xc600, 0x2640], color: 'red' };

//------------------------------------------------
// do the bit manipulation and iterate through each
// occupied block (x,y) for a given piece
//------------------------------------------------
function eachblock(type: Type, x: number, y: number, dir: DIR, fn: (x: number, y: number) => void) {
  let bit,
    row = 0,
    col = 0;
...
//-----------------------------------------------------
function occupied(type: Type, x: number, y: number, dir: DIR) {
  let result = false;
...
function unoccupied(type: Type, x: number, y: number, dir: DIR) {
...
function setVisualScore(n?: number) {
  vscore = n || score;
  invalidateScore();
}
...
function setBlock(x: number, y: number, type?: Type | null) {
  blocks[x] = blocks[x] || [];
...
const invalid = { court: false, next: false, score: false, rows: false };
...
function drawRows() {
  if (invalid.rows) {
    html('rows', rows.toLocaleString());
    invalid.rows = false;
  }
}

function drawPiece(ctx: CanvasRenderingContext2D, type: Type, x: number, y: number, dir: DIR) {
  eachblock(type, x, y, dir, function (x: number, y: number) {
    drawBlock(ctx, x, y, type.color);
  });
}

function drawBlock(ctx: CanvasRenderingContext2D, x: number, y: number, color: Color) {
  ctx.fillStyle = color;
